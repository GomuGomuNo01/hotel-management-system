/**
 * AdminComplaintsPage - /admin/reclamations
 *
 * Service client (interne) : liste des réclamations envoyées par les clients,
 * filtrables par statut. L'admin lit la réclamation et la marque « traitée »
 * avec une réponse optionnelle adressée au client.
 */
...
function HandleModal({ complaint, onClose, onDone }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setBusy(true);
    try {
      await adminApi.complaints.handle(complaint.id, { response: response.trim() || undefined });
...
export default function AdminComplaintsPage() {
  ...
  const fetchComplaints = useCallback(async () => {
    ...
    try {
      const res = await adminApi.complaints.list({ status: statusFilter || undefined, page, per_page: 20 });
  ...
  useAutoRefresh(['complaint.created', 'complaint.handled'], () => fetchComplaints());
        ...
        <div>
          ...
          {meta && (
            <p className="text-sm text-slate-500 mt-1">
              <span className="font-semibold text-slate-800">{meta.total}</span> réclamation{meta.total !== 1 ? 's' : ''} au total
            </p>
          )}
        </div>
        {badgeCounts.complaints > 0 && (
          <div className="flex-shrink-0 flex items-center gap-2 bg-orange-50 border border-orange-200 rounded-xl px-4 py-2">
            <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-orange-500 text-white text-xs font-black">
              {badgeCounts.complaints > 99 ? '99+' : badgeCounts.complaints}
            </span>
            <span className="text-sm font-semibold text-orange-700">
              {badgeCounts.complaints === 1 ? 'à traiter' : 'à traiter'}
            </span>
          </div>
        )}
      </div>
